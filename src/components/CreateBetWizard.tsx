...
import { X, Users, Swords, CheckCircle, TrendingUp, ChevronDown, Calendar, Clock, Search } from 'lucide-react';
...
export default function CreateBetWizard({
  ...
  // Theme based on destination
  const themeColor = betDestination === 'h2h' ? 'purple' : 'orange';
  const primaryColor = betDestination === 'h2h' ? '#9333ea' : '#ff6b35';
  const hoverColor = betDestination === 'h2h' ? '#a855f7' : '#ff8c5c';
  ...
  // Fetch friends (mock data - replace with real Firebase query)
  useEffect(() => {
    // TODO: Replace with real friends query from Firebase
    setFriends([
      { id: 'friend1', name: 'Phil Margevicius' },
      { id: 'friend2', name: 'Joe Wang' },
      { id: 'friend3', name: 'Joe Smith' },
    ]);
  }, []);
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      ...
      const betData: any = {
        ...
        status: betDestination === 'h2h' ? 'PENDING' : 'OPEN',
        picks: {},
        participants: [],
        createdAt: serverTimestamp(),
      };

      if (betDestination === 'group') {
        betData.groupId = selectedGroup;
        betData.betTheme = 'group';
      } else {
        betData.friendId = selectedFriend;
        betData.betTheme = 'friend';
      }
  ...
  // Validation
  const canProceedStep1 = betDestination !== null;
  ...
  const canProceedStep4 =
    title.trim() &&
    (betType === 'YES_NO' || (betType === 'OVER_UNDER' && line && parseFloat(line) % 1 === 0.5));
                ...
                >
                  <div className="flex items-start gap-3">
                    <Swords className={`w-6 h-6 mt-1 ${
                      betDestination === 'h2h' ? 'text-purple-500' : 'text-white'
                    }`} />
                    <div>
                      <div className={`text-[14px] font-semibold ${
                        betDestination === 'h2h' ? 'text-purple-500' : 'text-white'
            ...
            <div>
              ...
              <div className="space-y-3 mb-6">
                ...
                {betDestination === 'group' ? (
                  ...
                  <>
                    <div className="w-full p-4 rounded-lg border-2 bg-purple-500/10 border-purple-500">
                      <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Search..."
                          className="w-full h-10 pl-10 pr-4 bg-zinc-800 border-2 border-zinc-700 rounded-lg text-[14px] text-white placeholder-gray-500 focus:outline-none focus:border-purple-500"
                        />
                      </div>

                      {/* Recent Friends */}
                      <div>
                        <div className="text-[12px] text-white/60 font-semibold mb-2">
                          Recent Friends
                        </div>
                        <div className="space-y-2">
                          {filteredFriends.map(friend => (
                            ...
                            >
                              <div className={`text-[14px] font-semibold ${
                                selectedFriend === friend.id ? 'text-purple-500' : 'text-white'
                              }`}>
                                {friend.name}
                              </div>
                            </button>
            ...
            <div>
                ...
                >
                  <div className="flex items-start gap-3">
                    <CheckCircle className={`w-6 h-6 mt-1 ${
                      betType === 'YES_NO'
                        ? betDestination === 'h2h' ? 'text-purple-500' : 'text-[#ff6b35]'
                        : 'text-white'
                    }`} />
                    <div>
                      <div className={`text-[14px] font-semibold ${
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  style={{ backgroundColor: canProceedStep3 ? primaryColor : undefined }}
                  className={`flex-1 h-12 disabled:bg-zinc-800 disabled:text-white/50 disabled:cursor-not-allowed text-white text-[12px] font-semibold rounded-lg transition-colors`}
                  onMouseEnter={(e) => {
                    if (canProceedStep3) {
            ...
            <div>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder={betDestination === 'h2h' ? 'Broncos or Lost?' : 'Describe the bet...'}
                    style={{ borderColor: primaryColor }}
                    className="w-full h-12 px-4 bg-zinc-800 border-2 rounded-lg text-[14px] text-white placeholder-gray-500 focus:outline-none"
                    onFocus={(e) => e.currentTarget.style.borderColor = hoverColor}
                    onBlur={(e) => e.currentTarget.style.borderColor = primaryColor}
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    style={{ borderColor: primaryColor }}
                    className="w-full h-20 px-4 py-3 bg-zinc-800 border-2 rounded-lg text-[14px] text-white placeholder-gray-500 focus:outline-none resize-none"
                    onFocus={(e) => e.currentTarget.style.borderColor = hoverColor}
                    onBlur={(e) => e.currentTarget.style.borderColor = primaryColor}
                  />
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  style={{ backgroundColor: canProceedStep4 ? primaryColor : undefined }}
                  className="flex-1 h-12 disabled:bg-zinc-800 disabled:text-white/50 disabled:cursor-not-allowed text-white text-[12px] font-semibold rounded-lg transition-colors"
                  onMouseEnter={(e) => {
                    if (canProceedStep4) {
                      e.currentTarget.style.backgroundColor = hoverColor;
            ...
            <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    {['1', '5', '10', '20', '50'].map((amount) => (
                      <button
                        ...
                        className="flex-1 h-8 text-white text-[10px] font-semibold rounded transition-colors"
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className="w-full h-12 px-4 bg-zinc-800 border-2 border-zinc-700 rounded-lg text-[14px] text-white focus:outline-none"
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  style={{ backgroundColor: canProceedStep5 ? primaryColor : undefined }}
                  className="flex-1 h-12 disabled:bg-zinc-800 disabled:text-white/50 disabled:cursor-not-allowed text-white text-[12px] font-semibold rounded-lg transition-colors"
                  onMouseEnter={(e) => {
                    if (canProceedStep5) {
                      e.currentTarget.style.backgroundColor = hoverColor;
            ...
            <div>
              ...
              <div
                style={{
                  ...
                  backgroundColor: betDestination === 'h2h' ? 'rgba(147, 51, 234, 0.1)' : 'transparent'
                }}
                className="border-2 rounded-lg p-4 mb-6 space-y-2"
              >
                {betDestination === 'group' ? (
                  ...
                  <div className="flex justify-between">
                    <span className="text-[12px] text-white/60">H2H:</span>
                    <span className="text-[12px] text-white font-semibold">
                      {friends.find(f => f.id === selectedFriend)?.name} v. {user?.displayName || user?.email}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={loading}
                  style={{ backgroundColor: !loading ? primaryColor : undefined }}
                  className="flex-1 h-12 disabled:bg-zinc-800 disabled:cursor-not-allowed text-white text-[12px] font-semibold rounded-lg transition-colors"
                  onMouseEnter={(e) => {
                    if (!loading) {
                      e.currentTarget.style.backgroundColor = hoverColor;
                    }
